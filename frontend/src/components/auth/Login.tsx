...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      // Client-side validation for empty credentials
          ...
          >
            {({ isSubmitting }) => (
              <Form className="space-y-6">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className={`w-full px-4 py-3 rounded-lg border focus:outline-none focus:ring-2 transition-all duration-200 ${
                            meta.touched && meta.error ? "border-red-500 focus:ring-red-500" : "border-gray-300 focus:border-orange-500 focus:ring-orange-500"
                          }`}
                          placeholder="Enter your email"
                        />
                        {meta.touched && meta.error && <p className="mt-2 text-sm text-red-600">{meta.error}</p>}
                      </div>
                    )}
                  </Field>
                </div>
                ...
                {/* Submit Button */}
                <button type="submit" disabled={isSubmitting} className="w-full bg-blue-900 hover:bg-blue-800 text-white py-3 px-4 rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed">
                  {isSubmitting ? (
                ...
                {/* Google Login Button */}
                <button type="button" onClick={() => (window.location.href = `${import.meta.env.VITE_API_URL}/api/auth/google`)} className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors duration-200">
                ...
                <div className="text-center">
                  <p className="text-sm text-gray-600">
                    ...
                    <Link to="/register" className="text-orange-600 hover:text-orange-700 font-medium transition-colors duration-200">Sign up</Link>
                  </p>
                </div>
              </Form>
            )}
